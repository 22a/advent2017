...
; this fn taken from:
; https://stackoverflow.com/a/6450455
...
(defn -main []
  (def lines (filter not-empty (clojure.string/split-lines input)))
  (def lines-and-cols (map (fn [row] (clojure.string/split row #"\t")) lines))
  (def nums (map (fn [row] (map (fn [col] (read-string col)) row)) lines-and-cols))
      ...
      (map
        (fn
          ...
          (some
            #(when
               (=
                ; this is dirty, I'd like these maxes and mins to be `def`ed
                ; but don't know how I would do that in here. Perhaps up just
                ; before the `some` ?
                (mod (max (nth % 0) (nth % 1)) (min (nth % 0) (nth % 1)))
                0
                )
                ; and here too
               (quot (max (nth % 0) (nth % 1)) (min (nth % 0) (nth % 1)))
               )
            (combinations 2 row)
          )
        )
