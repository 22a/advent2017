...
fn find_state_index (mem_state: &Vec<usize>, prev_states: &Vec<Vec<usize>>) -> Option<usize> {
...
pub fn solution(mem_state: &Vec<usize>) -> usize {
    ...
    loop {
        ...
        match find_state_index(&current_state, &prev_states) {
            Some(index) => return num_transformations - index,
            None => (),
        }
        prev_states.push(current_state.to_owned());
    }
