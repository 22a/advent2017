...
const intRows = rowsAndColumns.map(row => row.map(column => parseInt(column)))

const checksum = intRows.reduce((sum, row) => {
  const extremes = row.reduce((acc, current) => ({
    low: Math.min(acc.low, current),
    high: Math.max(acc.high, current)
  }), {low: row[0], high: row[0]})

  return sum + (extremes.high - extremes.low)
}, 0)
